...
class Player_hat_guesser(AI):
    ...
    def play(self):
        ...
        if len(game.deck.cards) != 0:
            ...
            ### DEBUT WHAT TO DO ###
            what_to_do = 1000 #what_to_do ne doit initialement pas vérifier what_to_do <= number_cards
            if self.latest_turn_memory[2]!=-1 :
                what_to_do = self.latest_turn_memory[2]
            ...
            if what_to_do <= number_cards and self.latest_turn_memory[0]==0: #If the most recent recommendation was to play a card and no card has been played since the last hint, play the recommended card
                ...
                return('d%d'%(what_to_do-number_cards))
                #return ('c{}'.format(what_to_do))
